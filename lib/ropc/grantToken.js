...
module.exports = function grantToken(req, res, next, options) {
    ...
    function sendUnauthorizedError(description) {
        //res.header("WWW-Authenticate", "Basic realm=\"" + description + "\""); // This causes browser to show native username and password popup
    ...
    var origin = req.headers['origin'].replace(/https?\:\/\/|www\./gi, '') || 'hei';

    options.hooks.validateClient(clientId, clientSecret, origin, function (error, result) {
        if (error) {
            return next(error);
        }
